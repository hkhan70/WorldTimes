...
// Consent field
$consent_input = $consent_field == 'visible' || $consent_field == 'registered' && is_user_logged_in() || $consent_field == 'anonymous' && ! is_user_logged_in() ? '<div id="consent" class="form-group '.$inline_class.'"><label class="switch"><input type="checkbox" name="sform-consent" id="sform-consent" class="'.$consent_class.'" value="'.$checkbox_value.'" '.$consent_attribute.'><span class="slider round"></span></label><label for="sform-consent" class="control-label '.$consent_class.'">'.$consent_label.$required_consent_label.'</label></div>' : '';
...
// Submit field
$form .= '<div id="sform-submit-wrap" class="'.$submit_class.'"><button name="submission" id="submission" type="submit" class="'.$button_class.' btn btn-outline-secondary rounded-pill">'.$submit_label.'</button>'.$animation.'</div></form>'; 
