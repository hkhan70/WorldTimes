...
$attribute = '';
?>
<span id="new-release-message" style=""><?php echo apply_filters( 'sform_update', $attribute ) ?></span><div class="sform wrap">

<h1 class="backend"><span class="dashicons dashicons-editor-table"></span><?php esc_html_e( 'Form Editor', 'simpleform' ); global $wpdb; $table_name = "{$wpdb->prefix}sform_shortcodes"; $shortcode_data = $wpdb->get_var( "SELECT name FROM $table_name WHERE shortcode = 'simpleform'" ); $contact_form_name = $shortcode_data ? stripslashes(esc_attr($shortcode_data)) : esc_attr__( 'Contact Us Page','simpleform'); $widget = is_active_widget( false, false, 'sform_widget', true ) ? 'true' : 'false'; if ( $widget == 'true' ) { ?><div style="float: right;"><span style="font-size: 13px; padding-right: 10px;"><?php echo esc_html_e( 'Switch Form', 'simpleform' ) . ':' ; ?></span><span class="form-selector" style="float: right; padding: 0; top: 0;"><?php $id = isset( $_REQUEST['id'] ) ? absint($_REQUEST['id']) : '1'; $sform_widget = get_option('widget_sform_widget'); $widget_ids = array_keys($sform_widget); $sidebars_widgets = get_option('sidebars_widgets'); global $wp_registered_sidebars;?> <select name="widget" class="" id="widget" style="line-height: 34px; padding: 0 30px 0 20px; vertical-align: top;"><option value="1"><?php echo $contact_form_name; ?></option><?php foreach($widget_ids as $k) { if ( $k == is_int($k) ) { $shortcode_name = 'simpleform widget="'.$k.'"'; $shortcode_values = apply_filters( 'sform_form', $shortcode_name ); $default_name = __( 'Contact Form','simpleform'); if ( ! empty($shortcode_values['name']) && $shortcode_values['name'] != $default_name ) { $form_name = stripslashes(esc_attr($shortcode_values['name'])); } else { $form_name = __( 'Contact Form','simpleform') . ' (' . stripslashes(esc_attr($shortcode_values['area'])) .')'; } ?><option value="<?php echo $k; ?>" <?php selected( $id, $k ); ?>><?php echo $form_name; ?></option><?php } } ?></select></span></div><?php } ?></h1>

<?php
$form_type = isset( $_REQUEST['type'] ) && $_REQUEST['type'] == 'widget' ? $_REQUEST['type'] : '';
$widget_id = isset( $_REQUEST['id'] ) ? absint($_REQUEST['id']) : '';
if ( $widget_id == '' || $widget_id == '1' ) {
?>

<div class="page-description editorpage"><?php echo esc_html__( 'Change easily the way your contact form is displayed. Choose which fields to use and who should see them:','simpleform');?></div>

<?php
$sform_settings = get_option('sform_settings');
...
<tr class="outside"><th class="option"><span><?php esc_html_e('Shortcode','simpleform') ?></span></th><td class="shortcode text"><span id="shortcode" class="bold">[<?php echo $shortcode ?>]</span><input type="button" id="shortcode-copy" class="copy" value="<?php esc_html_e('Copy','simpleform') ?>"></td></tr>
		
<tr><th class="option"><span><?php esc_html_e('Form Name','simpleform') ?></span></th><td class="text"><input class="sform" name="form-name" placeholder="<?php esc_html_e('Enter a name for this Form','simpleform') ?>" id="form-name" type="text" value="<?php echo $contact_form_name; ?>"></td></tr>

<tr><th class="last option"><span><?php esc_html_e('Used in','simpleform') ?></span></th><td class="used-page">
		
<?php	
global $wpdb;
$table_post = $wpdb->prefix . 'posts';
$results = $wpdb->get_results("SELECT ID, post_title, guid, post_status FROM $table_post WHERE post_content LIKE '%[$shortcode]%' AND ( post_type = 'page' OR post_type = 'post') ");		    
$pages = '';		
if ( $results){
foreach ($results as $post) { 
if( $post->post_status == 'draft' || $post->post_status == 'publish' ) {
$publish_link = '<strong><a href="' . get_edit_post_link($post->ID) . '" target="_blank" style="text-decoration: none; color: #9ccc79;">' . __( 'Publish now','simpleform') . '</a></strong>';	
$post_status = $post->post_status == 'draft' ? __( 'Page in draft status not yet published','simpleform').'&nbsp;-&nbsp;' . $publish_link : sprintf( __('%1$s or %2$s the page content', 'simpleform'), '<strong><a href="' . get_edit_post_link($post->ID) .'" target="_blank" style="text-decoration: none;">'. $edit .'</a></strong>', '<strong><a href="' . get_page_link($post->ID) . '" target="_blank" style="text-decoration: none;">'. $view .'</a></strong>' );
$pages .= '<span class="bold">' . $post->post_title . '</span><span class="slug">[&nbsp;' . $post_status . '&nbsp;]<br>'; 
}
} }

if ( count($results) > 0 ) { $message = ''; }
else { $message = '<span class="bold">' . esc_attr__('Still not used. Create a new page or choose an existing one and add the shortcode','simpleform') . '</span>'; }
...
<input type="submit" class="submit-button" id="save-attributes" name="save-attributes"  value="<?php esc_html_e( 'Save Changes', 'simpleform' ) ?>"><?php  wp_nonce_field( 'ajax-verification-nonce', 'verification_nonce'); ?>
</form>
...
else { include_once('widget-editor.php'); }